  ...
  static Future<List<Map<String, dynamic>>> getMedicines({
    ...
    try {
      final response = await http
          .get(Uri.parse('$baseUrl/api/medicines$queryParams'))
      ...
      try {
        final response = await http
            .get(Uri.parse('$fallbackUrl/api/medicines$queryParams'))
            .timeout(const Duration(seconds: 4));
        if (response.statusCode == 200) {
          final decoded = jsonDecode(response.body);
          final List dataList = decoded['data'] ?? [];
        ...
        debugPrint(
          "API service unreachable. Falling back to local offline mock. Error: $e",
        );
      }
    }

    // Filter local data manually for search/category/classification to match DB behaviors offline
    return localMockMedicines
        .where((med) {
          final matchesSearch =
              search.isEmpty ||
