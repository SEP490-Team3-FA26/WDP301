...
export function PurchaseOrderCreate() {
  ...
  const [state, setState] = useState<{ prefilledMedicineId?: string, prefillPrItems?: any[] } | null>(null);

  useEffect(() => {
    if (typeof window !== "undefined") {
      const historyState = window.history.state;
      if (historyState && historyState.usr) {
        setState(historyState.usr);
      } else {
        const prefillStr = sessionStorage.getItem("purchase_order_prefill");
        if (prefillStr) {
          try {
            setState(JSON.parse(prefillStr));
            sessionStorage.removeItem("purchase_order_prefill");
          } catch (e) {
            console.error("Error parsing prefill from sessionStorage:", e);
          }
        }
      }
    }
