      ...
      <nav className="fixed w-full z-50 top-0 transition-all duration-300 py-5 px-6 border-b border-transparent sticky-nav">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4.5">
            <Link href="/customer/shop" className="font-bold text-xs uppercase tracking-wider text-slate-600 hover:text-[#0d6efd] transition-colors">
              Cửa Hàng
            </Link>
            <span className="w-px h-4 bg-slate-200"></span>
            <Link href="/interactions" className="font-bold text-xs uppercase tracking-wider text-[#0d6efd] hover:underline flex items-center gap-1">
              <BrainCircuit size={14} /> Tương tác thuốc AI
            </Link>
            <span className="w-px h-4 bg-slate-200"></span>

            {/* Cart Badge */}
            <Link ref={cartIconRef} href="/customer/cart" className="relative p-2 bg-slate-100 hover:bg-blue-50 text-slate-600 hover:text-[#0d6efd] rounded-xl transition-all">
          ...
          <div className="lg:col-span-7 flex flex-col items-start text-left">
            ...
            <div className="hero-fade flex flex-wrap gap-2.5 items-center">
              ...
              {categories.slice(1, 5).map((cat) => (
                <button
                  key={cat.value}
                  onClick={() => router.push(`/customer/shop?category=${encodeURIComponent(cat.value)}`)}
