...
import { PoTable } from "../../components/warehouse/PoTable";
    ...
    <div className="flex flex-col h-full bg-[#faf8ff] p-6 lg:p-8 overflow-y-auto">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col flex-1">
        <div className="p-4 border-b border-slate-200 flex flex-col sm:flex-row gap-4 items-center justify-between bg-slate-50">
          ...
          {type === "import" && activeSubTab === "po" && (
            <button
              onClick={handleCreate}
              className={`px-5 py-2.5 font-bold rounded-xl shadow-sm flex items-center gap-2 transition-colors bg-[#0057cd] hover:bg-[#00419e] text-white text-sm`}
            >
              <Plus size={18} />
        ...
        <div className="overflow-x-auto flex-1">
          {loading ? (
            ...
            <PoTable
              ...
              onSelectReceipt={(po) => setSelectedPoForReceipt(po)}
              onSelectReject={(po) => {
                setSelectedPoForReject(po);
                setRejectReason("");
              }}
              onSelectDetails={(po) => setSelectedPoDetails(po)}
            />
          )}
        </div>
      </div>
