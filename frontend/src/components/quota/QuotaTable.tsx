        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50 text-xs font-bold text-slate-500 uppercase tracking-wider">
              ...
              <th className="p-4 text-right">Tổng hạn mức</th>
              <th className="p-4 text-right">Đã sử dụng</th>
              <th className="p-4">Tỷ lệ đã dùng</th>
                ...
                <tr key={q._id} className="hover:bg-slate-50 transition-colors">
                  <td className="p-4 pl-6 font-semibold text-slate-900">
                    <div className="flex flex-col">
                      <span>{q.branchName || "Chi nhánh chưa cập nhật"}</span>
                  ...
                  <td className="p-4 text-right font-bold text-slate-900">{q.totalBudget.toLocaleString()}đ</td>
                  <td className="p-4 text-right font-bold text-slate-600">{(q.usedAmount || 0).toLocaleString()}đ</td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${
                      ...
                      {q.status === 'Active' ? 'Đang áp dụng' : 'Khóa'}
                    </span>
                  </td>
                  <td className="p-4 text-slate-500 max-w-[200px] truncate" title={q.note}>{q.note || "—"}</td>
            ...
            {filteredQuotas.length === 0 && (
              <tr>
                <td colSpan={8} className="p-12 text-center text-slate-400">
                  ...
                  Không tìm thấy dữ liệu hạn mức phân bổ nào.
                </td>
              </tr>
            )}
          </tbody>
